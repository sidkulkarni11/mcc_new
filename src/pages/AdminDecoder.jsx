...
--------------------------------------------- */
const TEST_META = {
    DEP: { label: "Depression Assessment", total: 75 },
    ANX: { label: "Anxiety Assessment", total: 75 },
    CGC: { label: "Career Goal Clarity", total: 75 },
    REL: { label: "Relationship Health Check", total: 75 },
    SEX: { label: "Sexual Well-Being Check", total: 75 },
    NEW: { label: "New Relationship Readiness", total: 75 },
};

/* --------------------------------------------
   Decode Helper (works with 5-char base64 slices)
--------------------------------------------- */
const decodeBlock = (segment) => {
    try {
        if (typeof window === "undefined") return null;

        // your generator uses 5 chars only, so decode needs padding
        let padded = segment;
...
const generateFeedback = (testId, score) => {
    ...
    const severity = getSeverity(score, meta.total);
...
export default function AdminDecoder() {
    ...
    const decodeNow = () => {
        try {
            ...
            const scoreRaw = decodeBlock(parts[1]);

            if (!testId || !scoreRaw) throw new Error();

            const score = Number(scoreRaw);
            if (isNaN(score)) throw new Error();

            const feedback = generateFeedback(testId, score);
            if (!feedback) throw new Error();

            // Safety check
            if (score < 0 || score > feedback.total) throw new Error();

            const pct = Math.round((score / feedback.total) * 100);

            const formatted = `
${feedback.label}
Score: ${score}/${feedback.total} (${pct}%)
        ...
        <div className="pt-32 px-6 min-h-screen bg-[#F4F7FF]">
            <div className="max-w-3xl mx-auto bg-white shadow-xl p-10 rounded-3xl border border-gray-200 animate-fadeIn">
                ...
                <p className="text-center text-gray-600 mb-6">
                    Decode any client’s assessment code instantly.
                </p>
                ...
                <textarea
                    ...
                    placeholder="Paste code (ex: REVQ-MTI-ABC)"
